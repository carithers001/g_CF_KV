...
这是一个单文件的 Cloudflare Workers / Pages 管理页，用于管理一个内部 Cloudflare 账号的 KV：登录后列出命名空间、按前缀浏览键、读取 UTF-8 文本并修改保存。
...
- 仅可修改读取到的已存在键；不提供新建、删除或 metadata/TTL 编辑。常规缺失键会被拒绝以避免意外创建，但读取与写入之间的删除/重建无法用 KV API 原子防止。
